...
function AdminLoginForm({ onSwitchToRegister, onSwitchToForgot }) {
  const { login, closeAuthModal, intendedPath, setIntendedPath } = useAuth();
  const { showToast } = useToast();
  const navigate = useNavigate();
...
function AdminForgotPasswordForm({ onSwitchToLogin }) {
  const { showToast } = useToast();
  const [email, setEmail] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
...
function AuthModal() {
  ...
  return (
    ...
    >
      ...
      {authModalView === "admin-forgot" && <h1>Reset Admin Password</h1>}
        ...
        {authModalView === "admin-login" && (
          <AdminLoginForm
            onSwitchToRegister={() => openAuthModal("admin-register")}
            onSwitchToForgot={() => openAuthModal("admin-forgot")}
          />
        )}
        {authModalView === "admin-register" && (
          <AdminRegisterForm onSwitchToLogin={() => openAuthModal("admin-login")} />
        )}
        {authModalView === "admin-forgot" && (
          <AdminForgotPasswordForm onSwitchToLogin={() => openAuthModal("admin-login")} />
        )}
      </div>
    </div>
  );
